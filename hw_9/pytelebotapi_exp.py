...
def calc(message):
    msg = bot.send_message(message.chat.id, "введите выражение типа (число) (действие) (число) "
                                            ...
                                            "доступны действия + - * / \n"
                                            "для комплексных чисел на данный момент доступны только  \n"
                                            "сложение и вычитание")
    bot.register_next_step_handler(msg, calculon)
def calculon(message):
    mess = message.text
    bot.send_message(message.chat.id,calulator.calc(mess))
